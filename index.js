...
var box = new Box(); //{timeInterval: 10}
...
Foxy.init = () => {
	...
	if (!nconf.get('google_token')) {
		...
		nconf.save(function(err) {
			...
			console.log('Configuration saved successfully. Time Zone');
		});
	}
	...
	if (!uuid) {
		...
		nconf.save(function(err) {
			...
			console.log('Configuration saved successfully. UUID');
		});
	}
	...
	if (!sCount) {
		...
		nconf.save(function(err) {
			...
			console.log('Configuration saved successfully. SILENCE COUNT');
		});
	} else {
	...
	if (!wwSensitivity) {
		...
		nconf.save(function(err) {
			...
			console.log('Configuration saved successfully.SENSITIVITY');
		});
	}
	...
	if (!audioGain) {
		...
		nconf.save(function(err) {
			...
			console.log('Configuration saved successfully.GAIN');
		});
	}
	...
	detector.on('silence', () => {
		if (foxy.state == stateEnum.LISTENING) {
			silenceCount++;
			if (silenceCount > maxSilence) {
				// Stop streaming and pause the microphone.
				console.log('Foxy Stopped Listening');
				Foxy.pause(foxy);
				foxy.state = stateEnum.PAUSED;
				parser.parseResults(Buffer.from(foxy.audioBuffer), function(status, utj) {
					if (status != 'ok') {
						logger.debug('parsing returned:' + status);
					}
					//Stick the utt in the correct bag in the box 
					if(typeof(utj) !== 'undefined'){
						if(box.addText(utj)){
							//Bags updated so update the UI
							var payload = {
								cmd: 'BGL',
								bags:box.getBags()
							};
							...
							//console.log("BEFORE SEND: ", JSON.stringify(payload));
							shimOptions.body = JSON.stringify(payload);
							rp(shimOptions);
					...
					console.log('Foxy Restarted Listening');
					foxy.audioBuffer = [];
					Foxy.resume(foxy);
	...
	detector.on('sound', (buffer) => {
		logger.debug('sound + State' + foxy.state);

		if (foxy.state == stateEnum.LISTENING) {
			logger.debug('State is LISTENING');
